...
from lens110_dance_obs import build_observation
...
# ---------------------------------------------------------------------------
ACTION_SCALE = 0.25
# 逐关节 scale (2026-08-20, CSV 13 顺序: 12腿+腰), 与训练 cfg 一致
PER_JOINT_SCALE = np.array([
    0.42, 0.25, 0.60, 0.47, 0.30, 0.25,      # L_hip_pitch, L_hip_roll, L_hip_yaw, L_knee, L_ankle_pitch, L_ankle_roll
    0.38, 0.25, 0.45, 0.60, 0.36, 0.25,      # R_hip_pitch, R_hip_roll, R_hip_yaw, R_knee, R_ankle_pitch, R_ankle_roll
    0.52,                                      # torso_yaw
], dtype=np.float64)
POLICY_HZ = 100.0
PHYSICS_HZ = 500.0
...
#   髋 100/8 (2026-08-20: 40/5 -> 80/8 -> 100/8, 实测最优:
#     100/8 脚距 0.421m 且 36s 不倒; 120/4 阻尼太低 2.45s 倒)
#   | 膝 40/5 | 踝 20/20
#   | 腰 100/5 | 肩/肘 100/5
KP = np.array([
...
# 2026-08-18 咨询结果: 未使用分段 PD, 默认关闭 (代码留存)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.25",
        choices=["0.25", "per_joint"],
        help="动作 scale: 0.25=统一 0.25 (v1 42500 策略, 默认); "
             "per_joint=逐关节 (2026-08-20 新训练策略)",
    )
    ...
    # 2) 接触配置: 对齐 Isaac
    #    - 2026-08-20: 不再加大 margin。实测 (headless 对照) margin=0.04 导致
    #      policy ON 时 1.2s 倒地 (friction/自碰撞无影响), MuJoCo 保持默认
    #      margin=0.001 数值稳定。
    ...
    # 默认关节角 (MuJoCo/CSV 顺序)
    # 2026-08-20 修复: default 必须取当前 motion 第 0 帧 (训练 default=init_state=帧0)。
    # 旧实现硬编码旧版帧0腿角 (踝 0.0409), 与过渡版 motion 帧0 (踝 0.0584)
    # 不一致, 导致动作 baseline 错位。
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=on_key) as viewer:
        ...
        while viewer.is_running() and frame < n_frames:
            ...
            # --- 策略推理 (100Hz) ---
            # 2026-08-20 修复: 每个策略步 (DECIMATION 个物理步) 必须推理一次。
            # 旧实现 policy_step 每策略步 +1, 但条件用 %DECIMATION, 导致实际
            # 10Hz 推理 (每 10 个策略步才推理), 是播放器比 headless 更早倒的原因。
            q_csv = data.qpos[csv_qpos].copy()          # 21 CSV 顺序
            qvel_csv = data.qvel[csv_dof].copy()
            robot_q_wxyz = data.qpos[3:7].copy()

            # 2026-08-19 修复: MuJoCo free joint qvel[3:6] 本身就是 body 系角速度,
            # 与训练观测 root_ang_vel_b 一致。旧实现再旋转一次变成 world 系,
            # 导致 obs[39:42] 与训练不匹配 (播放乱动/不稳的重要原因)。
            obs = build_observation(
                q_csv, qvel_csv, last_action, robot_q_wxyz, data.qvel[3:6].copy(),
                ref_pos_usd, ref_quat, ref_pos_w, frame, n_frames,
                USD_TO_CSV, CSV_TO_USD, clip_obs=CLIP_OBS,
            )
            out = sess.run(None, {"obs": obs.reshape(1, -1)})[0][0].astype(np.float32)
            out = np.clip(out, -CLIP_ACT, CLIP_ACT)
            last_action = out

            # 官方动作: 腿 = a*scale_i + default, 腰 = a*scale_i, 臂 = 参考开环
            q_des_csv = default_q_csv.copy()
            if args.scale_mode == "per_joint":
