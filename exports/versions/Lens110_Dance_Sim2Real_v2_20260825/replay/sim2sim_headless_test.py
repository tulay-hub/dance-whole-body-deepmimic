...
PER_JOINT_SCALE = np.array([
    # 2026-08-21: 统一全关节 0.25 (与训练 cfg 一致)
...
def main():
    ...
    ap.add_argument("--action_scale", type=float, default=None,
                    help="动作 scale 覆盖 (默认 0.25; 实验: 0.35/0.45/0.55)")
    ap.add_argument("--balance_kp", type=float, default=0.0,
                    help="下半身平衡: root 姿态误差 -> 踝/髋 target 增益 (默认 0=关)")
    ap.add_argument("--balance_kd", type=float, default=0.0,
                    help="下半身平衡: root 角速度 -> 踝/髋 target 增益")
    ap.add_argument("--balance_com_kp", type=float, default=0.0,
                    help="CoM 投影反馈: 质心-支撑中心误差 -> 踝/髋 target 增益")
    ap.add_argument("--balance_com_kd", type=float, default=0.0,
                    help="CoM 投影反馈: 质心速度项增益")
    args = ap.parse_args()
    ...
    per_joint_scale = PER_JOINT_SCALE.copy()
    if args.action_scale is not None:
        per_joint_scale[:] = args.action_scale
        print(f"[headless] action_scale 覆盖 -> {args.action_scale}")
    print(f"[headless] 逐关节 scale: {np.round(per_joint_scale, 2)}")
    ...
    for step in range(total_steps):
        ...
        q_des_csv[:12] = per_joint_scale[:12] * out[:12] + default_q_csv[:12]
        q_des_csv[12] = per_joint_scale[12] * out[12]
        arm_usd = [USD_JOINT_NAMES.index(n) for n in CSV_JOINT_ORDER[13:21]]
        q_des_csv[13:21] = ref_pos_usd[f][arm_usd]
        # 下半身平衡补偿 (2026-08-21 实验):
        #   root pitch/roll 与角速度 -> 踝 pitch/roll 与髋 pitch/roll target 修正
        if args.balance_kp != 0.0 or args.balance_kd != 0.0:
            qw, qx, qy, qz = data.qpos[3:7]
            roll = np.arctan2(2 * (qw * qx + qy * qz), 1 - 2 * (qx * qx + qy * qy))
            pitch = np.arcsin(np.clip(2 * (qw * qy - qz * qx), -1, 1))
            w = data.qvel[3:6]  # body 系角速度
            # 脚踝补偿 (前后=ank_p, 左右=ank_r): 姿态反向修正
            d_ank_p = args.balance_kp * pitch + args.balance_kd * w[1]
            d_ank_r = args.balance_kp * roll + args.balance_kd * w[0]
            q_des_csv[4] += d_ank_p   # L_ank_p
            q_des_csv[10] += d_ank_p  # R_ank_p
            q_des_csv[5] += d_ank_r   # L_ank_r
            q_des_csv[11] += d_ank_r  # R_ank_r
            # 髋补偿 (反向, 帮助重心回中)
            q_des_csv[0] -= 0.5 * d_ank_p  # L_hip_p
            q_des_csv[6] -= 0.5 * d_ank_p  # R_hip_p
            q_des_csv[1] -= 0.5 * d_ank_r  # L_hip_r
            q_des_csv[7] -= 0.5 * d_ank_r  # R_hip_r
        if args.balance_com_kp != 0.0 or args.balance_com_kd != 0.0:
            # CoM 世界位置 (subtree_com[0] = 全身质心)
            com = data.subtree_com[0]
            # 支撑中心: 左右脚踝 body 水平平均
            sup = 0.5 * (data.xpos[ankle_body["left_ankle_roll_link"]] +
                         data.xpos[ankle_body["right_ankle_roll_link"]])
            err_w = com[:2] - sup[:2]
            # 转到 root frame: 前后 (x) / 左右 (y)
            qw, qx, qy, qz = data.qpos[3:7]
            R = np.array([
            ...
            err_b = R.T @ np.array([err_w[0], err_w[1], 0.0])
            err_fwd, err_left = err_b[0], err_b[1]
            if "com_err_prev" not in globals():
                global com_err_prev
                com_err_prev = np.array([err_fwd, err_left])
            dt_s = 0.01
            vel_fwd = (err_fwd - com_err_prev[0]) / dt_s
            vel_left = (err_left - com_err_prev[1]) / dt_s
            com_err_prev[:] = [err_fwd, err_left]
            d_ank_p = args.balance_com_kp * err_fwd + args.balance_com_kd * vel_fwd
            d_ank_r = args.balance_com_kp * err_left + args.balance_com_kd * vel_left
            q_des_csv[4] += d_ank_p
            q_des_csv[10] += d_ank_p
            q_des_csv[5] += d_ank_r
            q_des_csv[11] += d_ank_r
            q_des_csv[0] -= 0.5 * d_ank_p
            q_des_csv[6] -= 0.5 * d_ank_p
            q_des_csv[1] -= 0.5 * d_ank_r
            q_des_csv[7] -= 0.5 * d_ank_r
